...
Asterisks indicating a type is a pointer should be “attached to” the variable name. **For example,** `NSString *text` **not** `NSString* text` or `NSString * text`, except in the case of constants (`NSString * const MDConstantString`).
...
```objc
@interface UIViewController (MDMediaPlaying)
...
```objc
@interface MDAdvertisement (private)
